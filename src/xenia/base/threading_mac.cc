...
timespec DurationToTimeSpec(std::chrono::duration<Rep, Period> duration) {
  auto nanoseconds =
...
// Thread interruption is done using SIGUSR1/SIGUSR2.
// lldb tip: process handle SIGUSR1 -s false
// lldb tip: process handle SIGUSR2 -s false
enum class SignalType { kThreadSuspend, kThreadUserCallback, k_Count };
